...
export default function Dashboard() {
  ...
  const handleClearAll = async () => {
    if (!confirm('Are you sure you want to delete all Call Flash records?')) return;
    setIsClearing(true);
    try {
      await axios.post('/voc-clear-all');
      toast.success('All Call Flash logs deleted successfully.');
    } catch (err) {
  ...
  const handleExport = async () => {
    ...
    const [filteredVocs, setFilteredVocs] = useState(vocs);

    useEffect(() => {
        setFilteredVocs(
            vocs.filter((voc: any) => {
                const vocDate = new Date(voc.created_at);

                const matchFrom = fromDate ? vocDate >= new Date(fromDate) : true;
                const matchTo = toDate ? vocDate <= new Date(toDate) : true;

                return matchFrom && matchTo;
            })
        );
    }, [fromDate, toDate, vocs]);
                ...
                <div className="flex gap-4 flex-wrap items-center">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="
                            ...
                            bg-gray-300 peer-checked:bg-green-500 relative
                    ...
                    <div className="flex gap-2 items-center">
                        <label className="whitespace-nowrap text-sm font-medium text-gray-700 dark:text-gray-200">
                        From:
                        </label>
                        <input
                        type="datetime-local"
                        ...
                        className="h-9 rounded-md border border-gray-300 dark:border-gray-600
                                    ...
                                    shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-300
                                    focus:ring-opacity-50 sm:text-sm px-2"
                        />
